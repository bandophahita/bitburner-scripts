...
/** @param {NS} ns */
export async function solveContracts(ns) {
    function log(msg) {
        ns.tprintf(`${msg}`);
        ns.print(msg);
    }
        ...
        for (let file of files) {
            ...
            switch (type) {
                ...
                case "Array Jumping Game":
                    answer = await solverArrayJumpingGame(ns, data)
                    break;
                case "Total Ways to Sum":
                    answer = await solverWaysToSum(ns, data);
                    break;
                case "Sanitize Parentheses in Expression":
                    answer = await sanitizeParentheses(ns, data);
                    break;
                default:
                    log(`ERROR Unhandled contract: ${type}`)
                    ns.alert(`Unhandled contract: ${type}`)
                    found = false
